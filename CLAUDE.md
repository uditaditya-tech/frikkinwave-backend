...
- **DEPLOYMENT STATE: UP as of 2026-08-21 — ~$0.26/hr.** Rebuilt and verified end to end this session: 3 ARM64 nodes across ap-south-1a/1b/1c, web x2 + relay + 4 consumer Deployments, Strimzi 1.1.0 / Kafka 4.2.1 (KRaft, 3 brokers, 26 topics all Ready), kube-prometheus-stack in `observability`, ALB + Route 53 live. 35 pods Running, **zero restarts**; `/api/health/`, `/api/docs/` and `/api/schema/` all 200. Database restored automatically from `frikkinwave-prod-final-f232bc9a`: 42 profiles, 720 reviews, and the denormalized rating rollup **verified drift-free against all 720 reviews, so `backfill_profile_ratings` was NOT needed** (that snapshot postdates the denormalization). A real `review.created` event was pushed through outbox → relay → Kafka → consumer in ~2s. Verify before assuming anything: `aws eks list-clusters --region ap-south-1`. Tear down with `./infra/scripts/eks-down.sh`. *(Update this bullet when the state changes — it is the first thing read each session, so a stale value here is worse than no value.)*
