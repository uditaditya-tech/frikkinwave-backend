...
- **DEPLOYMENT STATE: LIVE on EKS, last verified 2026-08-20.** 3 ARM64 nodes across ap-south-1a/1b/1c. In `frikkinwave`: web x2, **relay**, and **4 Kafka consumer Deployments** (`consume-{notifications,search,social,reviews}`). No Celery workers, no Redis. In `kafka`: Strimzi 1.1.0 + Kafka 4.2.1 (KRaft, 3 brokers, RF 3 / ISR 2, 26 topics). Image `e4b61b5-dirty.<ts>`, helm revision 3. **~$0.26/hr — tear down between sessions** with `./infra/scripts/eks-down.sh`. Verify before assuming: `aws eks list-clusters --region ap-south-1`. *(Update this bullet when the state changes.)*
  - **Two health signals exist; both are stopgaps until Prometheus.** A liveness probe on the relay's *loop progress* (a heartbeat file — the one deliberate exception to "never write to local disk", since it must die with the pod), and `manage.py check_outbox_lag` on a CronJob. Outbox lag is one number for every failure between `publish()` and the broker. Neither says anything about **consumer** lag — a message that reached Kafka and was never consumed leaves both clean.
  - **Failure behaviour is verified, not assumed** (2026-08-19 drill, see `KAFKA.md`): relay killed → events queued and drained on restart; one broker killed → transparent; **two of three** brokers killed → produces failed, events stayed pending, the relay survived, the alert fired, everything drained on recovery.
  - **The relay is the only path from the outbox to Kafka.** `publish()` dispatches nothing — a synchronous produce in the request path would add up to `KAFKA_FLUSH_TIMEOUT` to every request during a broker outage. **If `frikkinwave-relay` is down, no event is delivered at all.**
