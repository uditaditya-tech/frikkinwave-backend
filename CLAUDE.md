...
- **NOTHING IS CURRENTLY DEPLOYED — the app stack is torn down (~$0.50/mo).** Verified empty as of 2026-08-19: no EKS cluster, EC2, ALB, EBS, NAT gateway, or RDS instance. Only the **persistent `infra/dns/` stack** (Route 53 zone + ACM cert) and retained **RDS snapshots** survive. `api.frikkinwave.com` does not resolve to a running service.
  - **The ECS stack (`infra/terraform/`) is LEGACY.** It is unapplied (so $0) and kept in git as a fallback/reference. It is being replaced by **`infra/eks/`** — see `infra/eks/README.md` for the Phase 2 plan, cost model, and the two traps already hit (the EKS-version 6x-billing trap and the access-entry 409). **EKS Phase 1 is built and verified**, then torn down; bring it back with `./infra/scripts/eks-up.sh` (~15 min) and always tear it down with `./infra/scripts/eks-down.sh`.
  - **To restore the demo data**, restore RDS snapshot **`frikkinwave-prod-final-528299d1`** (2026-06-10, 20 GB): reference data + real profiles (`jazzcat`, `udit94`) + the full `demo-*` Phase 5 dataset (30 musicians, 870 follows, 720 reviews; any `demo-NN` logs in with `DemoPass123!`).
  - Historical note — what *was* live before teardown: Phase 5 Blocks A–C on image `668662f` (web + Celery worker + ElastiCache + RDS/pgvector on ECS Fargate), migrations applied through `social.0002` + `reviews.0001`. Phase 5 Block D (real-time messaging) is still NOT built. There is **no CD** (decision recorded in infra/README). *(Update this bullet when the deployment state changes.)*
