...
- **DEPLOYMENT STATE: LIVE on EKS, last verified 2026-08-20.** 3 ARM64 nodes across ap-south-1a/1b/1c. In `frikkinwave`: web x2, worker, notifications, search, redis, plus **4 Kafka consumer Deployments** (`consume-{notifications,search,social,reviews}`). In `kafka`: Strimzi 1.1.0 + Kafka 4.2.1 (KRaft, 3 brokers, RF 3 / ISR 2, 26 topics). Image `e4b61b5-dirty.<ts>`, helm revision 3. **~$0.26/hr — tear down between sessions** with `./infra/scripts/eks-down.sh`. Verify before assuming: `aws eks list-clusters --region ap-south-1`. *(Update this bullet when the state changes.)*
  - **`EVENT_TRANSPORT` is `celery`.** Kafka is fully wired and verified in both directions; the app still dispatches through Celery until the flag is flipped. Consumer Deployments run and idle meanwhile.
  ...
  - **Adding a Celery consumer means adding a Kafka subscription too.** `tests/test_architecture.py` asserts every topic with a Celery handler has one — otherwise flipping `EVENT_TRANSPORT=kafka` silently stops processing it, with no error anywhere.
  - **KAFKA.md stages 0-4 are DONE**, plus the security baseline and mTLS. Four consumer Deployments run `consume_events --group <app>` alongside the Celery workers; `EVENT_TRANSPORT` still defaults to `celery`, and both directions are verified live. **NEXT UP: stage 5** — delete Celery, its settings, the `--queues` workers, and the dependency.
  - **Every event topic needs a `.dlt` topic AND its ACL.** `authorization: simple` denies anything ungranted, so a missing DLT turns a dead-letter produce into a denied produce, a raised handler, and a stalled partition — the safety valve becoming the outage. `EVENT_TRANSPORT` defaults to `celery`, so the app still runs entirely on Celery until the flag is flipped — verified live in both directions. **NEXT UP: stage 4** (consumers subscribe by group instead of a central registry), then stage 5 (remove Celery).
