...
- **DEPLOYMENT STATE: TORN DOWN as of 2026-08-20 — $0/hr.** Verified: no clusters, no RDS, no volumes, no load balancers. The database is preserved in `frikkinwave-prod-final-002d5da3`, which the next `eks-up.sh` restores automatically. Verify before assuming anything: `aws eks list-clusters --region ap-south-1`. Bring it back with `./infra/scripts/eks-up.sh && ./infra/scripts/app-deploy.sh` (~20 min, ~$0.26/hr). *(Update this bullet when the state changes.)*
