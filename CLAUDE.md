...
- **DEPLOYMENT STATE: TORN DOWN as of 2026-08-21 — $0/hr.** Verified against AWS, not just the script's own report: no clusters, no RDS, no load balancers, no volumes. The database is preserved in `frikkinwave-prod-final-f232bc9a` (two manual snapshots exist; `eks-down.sh` warns past two), which the next `eks-up.sh` restores automatically. **The persistent stack survived and is doing its job**: the budget alarm now guards the window where orphaned resources would bill, and the SNS subscription stays confirmed so alerting needs no re-click on the next rebuild. Verify before assuming anything: `aws eks list-clusters --region ap-south-1`. Bring it back with `terraform -chdir=infra/dns apply` (idempotent, usually a no-op) then `./infra/scripts/eks-up.sh && ./infra/scripts/app-deploy.sh` (~20 min, ~$0.26/hr). *(Update this bullet when the state changes — it is the first thing read each session, so a stale value here is worse than no value.)*
