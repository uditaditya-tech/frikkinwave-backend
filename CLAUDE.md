...
- **DEPLOYMENT STATE: TORN DOWN as of 2026-08-19 — $0/hr.** `eks-down.sh` ran end to end: no orphaned load balancers or volumes, and the database is preserved in `frikkinwave-prod-final-38cea2fd`, which the next `eks-up.sh` restores automatically. Verify before assuming anything: `aws eks list-clusters --region ap-south-1`. Bring it back with `./infra/scripts/eks-up.sh && ./infra/scripts/app-deploy.sh`. *(Update this bullet when the state changes.)*
  ...
  - **KAFKA.md stages 0-3 and 4a are DONE**, plus the security baseline and mTLS. Stage 4b (consumer Deployments + live verification) is not. `EVENT_TRANSPORT` defaults to `celery`, so the app still runs entirely on Celery until the flag is flipped — verified live in both directions. **NEXT UP: stage 4** (consumers subscribe by group instead of a central registry), then stage 5 (remove Celery).
