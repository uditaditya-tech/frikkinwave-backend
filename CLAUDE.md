...
- **The app stack is currently DEPLOYED and live** — **Phase 2 is in prod** (`https://api.frikkinwave.com/api/health/` → 200 over HTTPS), running image `bb5f545` (current `main`): web + Celery worker + ElastiCache Redis + RDS with pgvector. Re-deployed 2026-06-05 after a deploy→destroy→redeploy cycle; a real semantic-search smoke test returned ranked results via OpenAI (key funded, in `terraform.tfvars`). DB holds reference seed data only (no user profiles). There is **no CD** (decision recorded in infra/README): deploying is manual — `push-image.sh <sha>` → `terraform apply -var image_tag=<sha>` → `run-migrations.sh` if there are new migrations. To tear down for cost savings: `terraform destroy` (now **takes a final RDS snapshot by default**, `db_skip_final_snapshot=false`; add `-var db_skip_final_snapshot=true` to wipe clean). DNS + cert always survive. A retained snapshot `frikkinwave-prod-phase2-20260606-0248` exists from the earlier teardown. *(Update this line when the deployment state changes.)*
