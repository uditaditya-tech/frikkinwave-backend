...
- **DEPLOYMENT STATE: TORN DOWN as of 2026-08-20 — $0/hr.** `eks-down.sh` ran end to end: no orphaned load balancers or volumes, and the database is preserved in `frikkinwave-prod-final-cd6558da`, which the next `eks-up.sh` restores automatically. Verify before assuming anything: `aws eks list-clusters --region ap-south-1`. Bring it back with `./infra/scripts/eks-up.sh && ./infra/scripts/app-deploy.sh` (~20 min, ~$0.26/hr). *(Update this bullet when the state changes.)*
