...
- **The app stack is currently DESTROYED** to save cost (only the ~$0.50/mo persistent DNS stack runs; `api.frikkinwave.com` does not resolve until re-deployed). **Phase 2 was deployed and verified live on 2026-06-05** (web + worker + ElastiCache + pgvector; a real semantic-search smoke test returned ranked results via OpenAI), then torn down. A **manual RDS snapshot `frikkinwave-prod-phase2-<ts>` was retained** before destroy — restore it via `snapshot_identifier` to recover that data. There is **no CD** (decision recorded in infra/README): deploying is manual — `push-image.sh <sha>` → `terraform apply -var image_tag=<sha>` → `run-migrations.sh` if there are new migrations (full first-Phase-2 runbook in infra/README "Phase 2: Redis broker + Celery worker"). The `OPENAI_API_KEY` goes in `terraform.tfvars` (git-ignored). To bring it back: re-apply per the runbook. To tear down: `terraform destroy` now **takes a final snapshot by default** (`db_skip_final_snapshot=false`); add `-var db_skip_final_snapshot=true` to wipe clean. DNS + cert always survive. *(Update this line when the deployment state changes.)*
