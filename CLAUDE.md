...
- **The app stack is currently DEPLOYED and live** — **Phase 2 is in prod** (`https://api.frikkinwave.com/api/health/` → 200 over HTTPS), running image `c543504` (current `main` HEAD): web + Celery worker + ElastiCache Redis + RDS with pgvector. Woken up 2026-06-06 from a parked/half-broken state (ECR was empty → tasks couldn't pull → 503; fixed by push-image + apply, then `run-migrations.sh` since the rebuilt RDS had no schema). DB holds reference seed data only (no user profiles). There is **no CD** (decision recorded in infra/README): deploying is manual — `push-image.sh <sha>` → `terraform apply -var image_tag=<sha>` → **always `run-migrations.sh`** (a fresh RDS has no schema). To tear down for cost savings: `terraform destroy` (now **takes a final RDS snapshot by default**, `db_skip_final_snapshot=false`; add `-var db_skip_final_snapshot=true` to wipe clean). DNS + cert always survive. Retained snapshots: `frikkinwave-prod-phase2-20260606-0248`, `frikkinwave-prod-final-0f41e8b8`. *(Update this line when the deployment state changes.)*
