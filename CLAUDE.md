...
- **The app stack is currently DEPLOYED and live** (`https://api.frikkinwave.com/api/health/` → 200; local `infra/terraform/terraform.tfstate` present). It runs the Phase 1 image — **the code on `main` is ahead of what's deployed** (2.1 Celery + 2.2 notifications are NOT in prod). There is **no CD**: CI only lints/tests/migrates; deploying is manual via infra/README "Updating the running app" (`push-image.sh <tag>` → `terraform apply -var image_tag=<tag>` → `run-migrations.sh` if there are new migrations). To tear down for cost savings: `cd infra/terraform && terraform destroy` (DNS + cert survive). *(Update this line when the deployment state changes.)*
  - **⚠️ Do NOT deploy 2.2+ until 2.9 lands.** Phase 2 added Celery with a Redis broker, but prod has no Redis/ElastiCache yet (deferred to 2.9). Deploying contact-request notifications without a broker would make send/accept fail at the `transaction.on_commit` `.delay()`. Provision ElastiCache + a Celery worker task def (2.9) before pushing any image that contains 2.2+, and **revisit adding CD at 2.9**.
